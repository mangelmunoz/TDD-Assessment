...
public class PersistenceTest {
    ...
    public void whenUserAgeinferiorTo9_getDiscounts_isCorrect(){
        ...
        Flight flight = persistenceService.addFlight(new Flight(1000, "IBE-1234","IBERIA", LocalDateTime.now().plusDays(2), LocalDateTime.now().plusDays(2).plusHours(2), new Country(1, ECountry.VALENCIA), new Country(2, ECountry.CORDOBA), 200.0, 50, true, 15.0, false, null, new ArrayList<>(), new ArrayList<>()));
    ...
    @Test
    public void whenHasLuggage_getPrice_returnsPriceWithLuggage(){
        Flight flight = persistenceService.addFlight(new Flight(1000, "IBE-1234","IBERIA", LocalDateTime.now().plusDays(2), LocalDateTime.now().plusDays(2).plusHours(2), new Country(1, ECountry.VALENCIA), new Country(2, ECountry.CORDOBA), 200.0, 50, true, 15.0, false, null, new ArrayList<>(), new ArrayList<>()));

        Passenger passenger = persistenceService.addPassengerToFlight(new Passenger(1000, "Test", "Test", "Nationality Test","email@email.com", "Identification test", 18, true, null, flight));

        Assertions.assertEquals(passenger.getPrice(), flight.getPrice()+flight.getLuggagePrice());
    }
